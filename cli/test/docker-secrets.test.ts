...
test(
  "docker + local sandbox addresses core by container name, overriding the operator's PUBLIC_API_URL",
  { timeout: 60_000 },
  async () => {
    const dir = mkdtempSync(join(tmpdir(), "qm-docker-local-coreurl-"));
    const priorPath = process.env.PATH;
    const priorDb = process.env.DATABASE_URL;
    const log = console.log,
      warn = console.warn;
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "PUBLIC_API_URL=https://wrong.example.test",
        ].join("\n"),
      );
      const fake = fakeDocker(dir);
      process.env.PATH = `${dir}:${priorPath}`;
      ...
      const { config } = loadConfigAt(join(dir, CONFIG_FILENAME));
      await dockerUp(config, dir, {});
      const combined = readFileSync(fake.argvLog, "utf8") + readFileSync(fake.envCopy, "utf8");
      assert.ok(
        combined.includes("PUBLIC_API_URL=http://qm-sekritlocal-core:8080"),
        "core is reachable from local sandboxes by container name",
      );
      assert.ok(
        !combined.includes("wrong.example.test"),
        "the operator-supplied URL is ignored on the containerised local path",
      );
    } finally {
      console.log = log;
      console.warn = warn;
...
test(
  "docker + local sandbox does not require the operator to set PUBLIC_API_URL at all",
  { timeout: 60_000 },
  async () => {
    const dir = mkdtempSync(join(tmpdir(), "qm-docker-local-nourl-"));
    const priorPath = process.env.PATH;
    const priorDb = process.env.DATABASE_URL;
    const log = console.log,
      warn = console.warn;
    try {
      ...
      const combined = readFileSync(fake.argvLog, "utf8") + readFileSync(fake.envCopy, "utf8");
      assert.ok(
        combined.includes("PUBLIC_API_URL=http://qm-sekritnourl-core:8080"),
        "the derived URL is delivered without an operator-supplied value",
      );
    } finally {
      console.log = log;
      console.warn = warn;
